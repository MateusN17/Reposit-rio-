...
class Telalogin(BoxLayout):
    ...
    def open_popup3(self):
        box = BoxLayout(orientation="vertical", padding=10, spacing=10)
        botoes = BoxLayout(padding=10, spacing=10)
        sim = Button(text="Sim", on_release=self.close_popup)
        nao = Button(text="Não", on_release=self.close_popup)
        botoes.add_widget(sim)
        botoes.add_widget(nao)
        box.add_widget(botoes)
        self.popup = Popup(title="Você quer trocar sua senha?", content=box, size_hint=(None,None), size=(200,150))
        self.popup.open()
